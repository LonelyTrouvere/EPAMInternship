...
const CourseForm = ({setCoursePage}) => {
    ...
    return (
        ...
        <div className="spacing">
            <Input labelText='Title' type='text' placeholder='Enter title...' onChange={handleTitel}/>
            <Button text='Return' className='float-right' onClick={onReturn}/>
            <Button text='Create course' className='float-right' onClick={createCourse}/>
        </div>
        <Input labelText='Description' type='textarea' placeholder='Enter description' className='form-text-area' onChange={handleDescription}/>
        <div className="authors-area">
            <div className="create-author">
                <h3 className="form-header">Add author</h3>
                <Input type='text' labelText='Author name' placeholder='Enter author name...' onChange={handleNewAuthor}/>
                <Button text='Create author' className='author-create-button' onClick={addNewAuthor}/>
            </div>
                    ...
                    .map((item) => {
                        return (
                            <div key={item.id} className="add-author">
                                ...
                                <Button text='Add author' className='float-right' onClick={() => addCourseAuthor(item)}/>
                            </div>
                        )
                    })
                }
                ...
                <h3 className="form-header">Course authors</h3>
                {
                    ...
                    courseAuthors.map((item) => {
                        return (
                            <div key={item.id} className="add-author">
                                <p className="float-left author-name">{item.name}</p>
                                <Button text='Delete author' className='float-right' onClick={() => removeAuthor(item)}/>
                            </div>
                        )
                    })
                }
            ...
            <div className="duration-area">
                ...
                <Input labelText='Duration' type='number' placeholder='Enter duration in minutes...' className='spacing' onChange={handleDuration}/>
                <p className="duration"><b>Duration:</b> {hours}:{minutes} hours</p>
            </div>
        </div>
        </form>
    );
